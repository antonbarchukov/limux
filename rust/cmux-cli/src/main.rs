...
use cmux_protocol::{V2Request, V2Response};
...
fn wait_signal_path(name: &str) -> PathBuf {
    let sanitized: String = name
        ...
        .collect();
    PathBuf::from(format!("/tmp/cmux-wait-for-{}.sig", sanitized))
}
...
async fn run_rename_workspace_like(
    ...
    let workspace = parse_opt(args, "--workspace").or_else(|| env::var("CMUX_WORKSPACE_ID").ok());
...
async fn run_rename_tab(client: &mut Client, args: &[String]) -> Result<Value> {
    ...
    let tab = parse_opt(args, "--tab")
        .or_else(|| env::var("CMUX_TAB_ID").ok())
        .unwrap_or_default();
...
async fn run_tab_action(client: &mut Client, args: &[String]) -> Result<Value> {
    if parse_flag(args, "--help") {
        return Ok(json!({
            "help": "Usage: cmux tab-action --action <name> [--workspace <id|ref>] [--tab <id|ref>] [--title <text>] [--url <url>]\nTarget tab:\n  --tab tab:<n>       Stable tab reference alias\n  --tab surface:<n>   Surface alias (legacy-compatible)\nExamples:\n  cmux tab-action --workspace workspace:2 --tab tab:1 --action pin\n  cmux tab-action --tab tab:3 --action mark-unread"
        }));
    }
    ...
    let workspace = parse_opt(args, "--workspace").or_else(|| env::var("CMUX_WORKSPACE_ID").ok());
    let tab = parse_opt(args, "--tab").or_else(|| env::var("CMUX_TAB_ID").ok());
    ...
    let output = match sub.as_str() {
        ...
        "screenshot" => {
            ...
            let mut path = get_string(&payload, &["path"])
                .unwrap_or_else(|| "/tmp/cmux-browser-shot.png".to_string());
